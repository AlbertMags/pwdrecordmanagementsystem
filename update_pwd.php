...
try {
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] == 0) {
        ...
        // Use a unique name to prevent cache issues or overwriting
    ...
    // Note: disability_type and barangay_id are treated as integers
    $fields = [
        ...
        'disability_type'          => $_POST['disability_type'] ?? '', // Kept as string for bind_param flexibility or change logic below
    ...
    // Add photo to update if a new one was uploaded
    if ($photo_filename) {
        $setClause .= "photo = ?, "; // Ensure this matches your column name 'photo'
        $types .= "s";
    ...
    foreach ($fields as $column => $value) {
        $setClause .= "$column = ?, ";
        // Assign types: 'i' for integer columns, 's' for strings
        if (in_array($column, ['barangay_id'])) {
            $types .= "i";
        } else {
            $types .= "s";
        }
