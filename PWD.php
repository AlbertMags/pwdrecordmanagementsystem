...
// Fetch barangays and disabilities for filtering
...
$result = $conn->query("SELECT pwd.*, barangay.brgy_name, disability_type.disability_name 
                        FROM pwd 
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Updated Filter Container for Single Line */
        .filters {
            background: #f8f9fa;
            padding: 15px;
            border-radius: 8px;
            ...
            gap: 15px;
            align-items: center;
            overflow-x: auto;   /* Scroll if screen is very small */
        }
        ...
        .edit-btn { background-color: #07a0e2; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer; color: white; }
        .delete-btn { background-color: #dc3545; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer; color: white; }
        .approve-btn, .accept-btn { background-color: #28a745; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer; color: white; }
        .check-btn { background-color: #0056b3; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer; color: white; margin-right: 5px; }

        #pwdTableBody tr { display: none; }
        ...
        .pwd-form-container, .assessment-container {
            background: #fff; 
            margin: 2vh auto; 
            width: 95%; 
            max-width: 1100px; 
        ...
        .doh-form-wrapper h3 {
            ...
            border-left: 4px solid #0056b3; margin: 20px 0 10px 0; text-transform: uppercase;
        ...
        .iframe-wrapper { flex: 1; width: 100%; }
        #assessmentFrame, #viewFrame { width: 100%; height: 1600px; border: none; overflow: hidden; }
...
<body>
    ...
    <header class="top-nav">
        <div class="nav-brand-wrapper">
            ...
            <div class="nav-text-stack">
                ...
                <p class="nav-sub">Screen, verify, and approve pending PWD registrations.</p>
            </div>
        </div>
    </header>
            ...
            <div class="filters">
                <div class="filter-group">
                    ...
                    <select id="barangayFilter">
                        ...
                        <?php $barangayResult->data_seek(0); while ($row = $barangayResult->fetch_assoc()) { ?>
                ...
                <div class="filter-group">
                    <label>Filter by Disability:</label>
                ...
                <div class="filter-group">
                    ...
                    <select id="statusFilter">
                        ...
                        <option value="Pending" <?php echo (strtolower($active_tab) == 'pending') ? 'selected' : ''; ?>>Pending Applicants</option>
                        <option value="Screening" <?php echo (strtolower($active_tab) == 'screening') ? 'selected' : ''; ?>>Under Screening</option>
                        <option value="For Approval" <?php echo (strtolower($active_tab) == 'for approval' || $active_tab == 'approval') ? 'selected' : ''; ?>>For Approval</option>
                    </select>
                </div>
            </div>
            ...
            <table width="100%">
                ...
                <?php while ($row = $result->fetch_assoc()) { ?>
                    <tr data-barangay="<?php echo $row['barangay_id']; ?>" 
                        ...
                        <td>
                            <span class="clickable-name view-pwd-trigger" data-id="<?= $row['id']; ?>">
                        ...
                        <td>
                            ...
                            <?php } elseif ($row['status'] === 'Screening') { ?>
                                <span style="font-style:italic; color:gray;">In Screening...</span>
                            <?php } elseif ($row['status'] === 'For Approval') { ?>
                                <button class="edit-btn" data-id="<?php echo $row['id']; ?>">Edit</button>
                                <button class="check-btn" data-id="<?php echo $row['id']; ?>">🔍 Check</button>
                            <?php } ?>
                        </td>
                    </tr>
                <?php } ?>
    ...
    <div class="pwd-modal-backdrop" id="addPWDModal">
        <div class="pwd-form-container">
            <span class="close" style="position:absolute; right:20px; top:10px; cursor:pointer; font-size:28px; z-index:100;">&times;</span>
                    ...
                    <div class="form-row">
                        ...
                        <div><label>7. CIVIL STATUS:</label>
                            <select name="civil_status" id="civil_status" required>
                                <option value="Single">Single</option><option value="Married">Married</option><option value="Widowed">Widowed</option><option value="Separated">Separated</option><option value="Cohabitation">Cohabitation (Live-in)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="disability-split-container">
                        <div class="split-left">
                            ...
                            <div class="disability-grid">
                                ...
                                while($d = $disabilityResult->fetch_assoc()) {
                                    echo "<div class='disability-item'><input type='radio' name='disability_type' value='{$d['id']}' required> ".htmlspecialchars($d['disability_name'])."</div>";
                                }
                                ?>
                            </div>
                        </div>
                    ...
                    <div class="form-row">
                        ...
                        <div><label>BARANGAY:</label>
                            <select name="barangay_id" id="barangay_id" required>
                                <?php $barangayResult->data_seek(0); while ($row = $barangayResult->fetch_assoc()) { echo "<option value='{$row['id']}'>".htmlspecialchars($row['brgy_name'])."</option>"; } ?>
                            </select>
                        </div>
                    </div>
                    <div class="form-row">
                    ...
                    <div class="form-row">
                        <div style="flex:1;"><label>FATHER'S NAME (L, F, M):</label>
                            <div class="name-group">
                                <input type="text" name="father_lastname" id="father_lastname" placeholder="Last">
                                <input type="text" name="father_firstname" id="father_firstname" placeholder="First">
                                <input type="text" name="father_middlename" id="father_middlename" placeholder="Middle">
                            </div>
                        </div>
                    </div>
                    <div class="form-row">
                        <div style="flex:1;"><label>MOTHER'S NAME (L, F, M):</label>
                            <div class="name-group">
                                <input type="text" name="mother_lastname" id="mother_lastname" placeholder="Last">
                                <input type="text" name="mother_firstname" id="mother_firstname" placeholder="First">
                                <input type="text" name="mother_middlename" id="mother_middlename" placeholder="Middle">
                            </div>
                        </div>
                    </div>
                    <div class="form-row">
                        <div style="flex:1;"><label>GUARDIAN'S NAME (L, F, M):</label>
                            <div class="name-group">
                                <input type="text" name="guardian_lastname" id="guardian_lastname" placeholder="Last">
                                <input type="text" name="guardian_firstname" id="guardian_firstname" placeholder="First">
                                <input type="text" name="guardian_middlename" id="guardian_middlename" placeholder="Middle">
                            </div>
                        </div>
                    </div> 
    ...
    <div class="pwd-modal-backdrop" id="checkMedicalModal">
        <div class="assessment-container">
            <div style="background: #0056b3; color: white; padding: 15px; display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; z-index: 10;">
                <h3 style="margin:0;">Verification: Medical Assessment (Form 2)</h3>
                <span class="close-check" style="cursor:pointer; font-size: 24px;">&times;</span>
            </div>
            <div class="iframe-wrapper">
                <iframe id="assessmentFrame" scrolling="no"></iframe>
            </div>
            <div class="modal-footer">
                <button type="button" class="delete-btn close-check" style="padding: 10px 30px;">Close</button>
                <button type="button" id="modalFinalApproveBtn" class="approve-btn" style="padding: 10px 30px;background-color:#0056b3; font-weight: bold;">✅ Final Approve & Verify</button>
            </div>
        </div>
    </div>

    <div class="pwd-modal-backdrop" id="viewPWDModal">
        <div class="assessment-container">
            <div style="background: #0056b3; color: white; padding: 15px; display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; z-index: 10;">
                <h3 style="margin:0;">View PWD Details</h3>
                <span class="close-view" style="cursor:pointer; font-size: 24px;">&times;</span>
            </div>
            <div class="iframe-wrapper">
                <iframe id="viewFrame" scrolling="no"></iframe>
            </div>
        </div>
    </div>
...
$(document).ready(function () {
    ...
    // --- FILTERS ---
    $("#barangayFilter,#disabilityFilter,#statusFilter").on("change", applyFilters);
    function applyFilters() {
        let b = $("#barangayFilter").val();
        let d = $("#disabilityFilter").val();
        let s = $("#statusFilter").val();
        $("#pwdTableBody tr").each(function(){
            let rb = $(this).attr("data-barangay");
            let rd = $(this).attr("data-disability");
            let rs = $(this).attr("data-status");
            if ((b==="" || rb==b) && (d==="" || rd==d) && (s==="" || rs==s)) { $(this).css("display", "table-row"); } 
            else { $(this).css("display", "none"); }
        });
        renumber();
    }
    function renumber() {
        $("#pwdTableBody tr:visible").each(function(i){ $(this).find(".row-number").text(i+1); });
    }
    
    // UPDATED: Filter will default to "Pending" on load
    applyFilters(); 
    ...
    $(".close, .close-check, .close-view").click(() => {
        $(".pwd-modal-backdrop").fadeOut();
        $("#viewFrame, #assessmentFrame").attr("src", ""); 
    });

    // --- VIEW PWD MODAL TRIGGER ---
    $(document).on("click", ".view-pwd-trigger", function() {
        let id = $(this).data("id");
    ...
    $(document).on("click", ".check-btn", function() {
        currentCheckId = $(this).data("id");
        $("#assessmentFrame").attr("src", "doctor/view_assessment.php?id=" + currentCheckId);
        $("#checkMedicalModal").fadeIn();
    });

    // --- FINAL APPROVAL ---
    $("#modalFinalApproveBtn").click(function() {
        if(confirm("Confirm final verification? This will move the PWD to the Official Registry.")) {
            $.post("finalize_pwd.php", {id: currentCheckId}, function(res){
                if(res.includes("success")) {
                    var parts = res.split("|");
                    alert("PWD Moved to Official List!\n\nEmail: " + parts[1] + "\nPassword: " + parts[2]);
                    location.reload();
    ...
    $("#pwdRegistrationForm").submit(function (e) {
        ...
        let actionUrl = $("#pwd_id").val() === "" ? "add_pwd.php" : "update_pwd.php";
        $.ajax({
            url: actionUrl, type: "POST", data: formData, contentType: false, processData: false, dataType: "json",
            success: function (res) {
                alert(res.message);
                if (res.status === "success") location.reload();
            },
            error: function(xhr) { alert("SAVE ERROR:\n" + xhr.responseText); }
        });
    });
    ...
    $(document).on("click", ".edit-btn", function() {
        let id = $(this).data("id");
        $.ajax({
            url: "get_pwd_details.php", type: "POST", data: {id: id}, dataType: "json",
            success: function(res) {
                if(res.status === "success") {
                    ...
                    if(d.disability_type) { $("input[name='disability_type'][value='"+d.disability_type+"']").prop("checked", true); }

                    $("input[name='disability_cause']").prop("checked", false);
                    if(d.disability_cause) { $("input[name='disability_cause'][value='"+d.disability_cause+"']").prop("checked", true); }
                    ...
                    if(d.accomplished_by_type) { $("input[name='accomplished_by_type'][value='"+d.accomplished_by_type+"']").prop("checked", true); }
    ...
    // --- DELETE & APPROVE ---
    $(document).on("click", "#pwdTableBody .delete-btn", function () {
        let id = $(this).data("id");
        if(confirm("Permanently remove this PWD record?")) {
            $.post("delete_pwd.php", {delete_id:id}, (res) => { if(res.trim()==="success") location.reload(); }); 
        }
    });
    
    $(document).on("click", "#pwdTableBody .approve-btn", function () {
        let id = $(this).data("id");
        if (confirm("Are you sure you want to move this applicant to Screening?")) {
            $.post("approve_pwd.php", { id: id }, (res) => {
                if (res.trim() === "success") { alert("Applicant successfully moved to Screening!"); location.reload(); } 
                else { alert("Error: " + res); }
            });
        }
    });
