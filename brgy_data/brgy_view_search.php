<?php
// Secure connection to the database (one folder up)
include("../db_connect.php");

// Ensure session is started to check barangay_id
if (session_status() === PHP_SESSION_NONE) { session_start(); }

if (!isset($_GET['id']) || empty($_GET['id'])) {
    echo "<div style='padding:50px; text-align:center; font-family:sans-serif;'>
            <h2>Invalid Request</h2>
            <a href='/PWD/brgy_data/brgy_dashboard'>Go Back</a>
          </div>";
    exit;
}
...
$query = "SELECT pwd.*, barangay.brgy_name, disability_type.disability_name 
          ...
          WHERE pwd.id = ? AND pwd.barangay_id = ?";

$stmt = $conn->prepare($query);
$stmt->bind_param("ii", $pwd_id, $my_brgy_id);
$stmt->execute();
...
if (!$pwd) {
    echo "<div style='padding:50px; text-align:center; font-family:sans-serif;'>
            <h2>PWD Record Not Found</h2>
            <p>The record might not belong to your Barangay or has been removed.</p>
            <a href='/PWD/brgy_data/brgy_dashboard'>Go Back</a>
          </div>";
    exit;
}
...
if (!function_exists('format_val')) {
    function format_val($val) {
        if (empty($val) || $val == 'None' || $val == 'N/A') return '<span style="color:#888; font-style:italic;">None recorded</span>';
...
<style>
    ...
    @media print {
        ...
        .sidebar, #sidebar, .brgy-sidebar-wrapper { display: none !important; }
        .view-profile-page-wrapper {
            position: absolute; left: 0; top: 0; width: 100% !important; margin: 0 !important;
            padding: 1.5cm !important; background: white !important;
        }
...
<div class="view-profile-page-wrapper">
    <div class="doh-card">
        <div class="doh-header">
            <img src="/PWD/uploads/doh.png" class="doh-logo" alt="DOH Logo">
        ...
        <div class="main-row">
            ...
            <div class="photo-sidebar">
                <div class="photo-box">
                    <?php 
                        // Using absolute server path for check and absolute URL for display
                        $check_path = $_SERVER['DOCUMENT_ROOT'] . "/PWD/uploads/profile_pics/" . basename($pwd['photo']);
                        $display_path = "/PWD/uploads/profile_pics/" . basename($pwd['photo']);
                        if (empty($pwd['photo']) || !file_exists($check_path)) { 
                            $display_path = "/PWD/uploads/profile_pics/default.png"; 
                        }
                    ?>
                    <img src="<?php echo $display_path; ?>?t=<?php echo time(); ?>" alt="Profile Photo">
                </div>
            ...
            <div class="assessment-grid">
                ...
                <div class="col">
                    ...
                    <div class="field" style="margin-top:15px;">
                        ...
                        <div class="value">
                            <strong>Etiology:</strong> <?php echo htmlspecialchars($pwd['assessment_etiology'] ?? 'N/A'); ?> <br>
        ...
        <div class="button-group">
            <a href="/PWD/brgy_data/brgy_dashboard" class="btn btn-back">Back to Dashboard</a>
