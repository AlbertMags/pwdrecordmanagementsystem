...
try {
    // 1. Collect data from form (Synced with Main System logic)
    ...
    // Hardcoded location data as per main system
    ...
    // 2. IMAGE UPLOAD LOGIC (Targeting common uploads folder)
    $db_image_name = ""; 
    if (!empty($_FILES['profile_picture']['name'])) {
        ...
        $file_name = "BRGY_" . time() . "_" . uniqid() . "." . $file_extension;
    ...
    // 44 total parameters for the 44 '?' markers
    $stmt->bind_param(
    ...
    if ($stmt->execute()) {
        // 4. NOTIFICATION LOGIC (Alerting Admin of Barangay Submission)
        $full_name = trim($first_name . ' ' . $last_name);
        $notif_msg = "Barangay Application: $full_name has been submitted.";
        
        $notif_sql = "INSERT INTO notifications (barangay_id, message, read_by_admin, created_at) VALUES (?, ?, 0, NOW())";
